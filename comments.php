...
?>
    ...
    <?php if ( have_comments() ) : ?>
        ...
        <h2><?php comments_number("0 ".__('Comments','tfuse'), "1 ".__('Comment','tfuse'), "% ".__('Comments','tfuse')); ?><?php _e(' Added', 'tfuse') ?></h2>
        <a href="#addcomments" class="link-add-comment anchor"><?php _e( 'add your reply', 'tfuse' );?></a>
        <ol>
            <?php
...
$args = array(
    ...
    'comment_field' => ' <div class="row">
                            <label class="label_title"><strong>' . __( 'Comment', 'tfuse' ) .':</strong></label>
    ...
    'fields' => apply_filters( 'comment_form_default_fields', array(
            ...
            'author' =>'<div class="row alignleft field_text">
                        <label class="label_title"><strong>' . __( 'Name', 'tfuse' ) . ':</strong></label>
                        <input type="text" name="author" class="inputtext required" id="name" value="' . esc_attr( $commenter['comment_author'] ) .'" tabindex="1" size="30"' . $aria_req . '/>
                    </div>',

            'email' =>'<div class="row alignleft field_text omega">
                            <label class="label_title"><strong>' . __( 'Email', 'tfuse' ) . '</strong><small>'. __('(never published)','tfuse').'</small>:</label>
                            <input type="text" name="email" class="inputtext required" id="email" value="' . esc_attr(  $commenter['comment_author_email'] ) .'" tabindex="2" size="30"' . $aria_req . ' />
                        </div>',
        )
    ),
);
