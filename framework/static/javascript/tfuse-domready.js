...
function tf_optionize() {
            ...
            $(this).one('focus',function() {
                ...
                $('#'+input_id).suggest( 'admin-ajax.php?action=tfuse_get_suggest&type='+type+'&name='+name,
                {
                    ...
                    onSelect: function()
                    {
                        ...
                        if( arrvals.length === ( limit ) ) {
                            $(this).blur().val('You cannot set more then ' + limit + ' items');
                            return;
                        }
    ...
    if(typeof tf_optionize.one_time=='undefined') {
            ...
            $('.postbox').on('click', '.multi_upload2_button', function(){
                ...
                $.ajax({
                    ...
                    success: function(r){
                        ...
                        window.skip_send_to_editor = true;

                        loading.hide();

                        $('#TB_window,#TB_overlay,#TB_HideSelect').one('unload'+ eventsNamespace, killTheUnloadEvent);

                        $('#TB_window').one('tb_unload'+ eventsNamespace, function(){
                            var $this = $(this);

                            window.skip_send_to_editor = false;
        ...
        window.send_to_editor = function(html)
        {
            ...
            {
                if ( ! window.original_send_to_editor(html) ) {
                    if ( window.tb_remove ) {
                        try { window.tb_remove(); } catch( e ) {}
                    }
                }
