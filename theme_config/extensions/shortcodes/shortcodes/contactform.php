...
function tf_contactform_shortcode($atts){
    ...
    if($form_exists){
        $out.=' <div class="add-comment contact-form" id="addcomments">';
        $out .='<div class="add-comment-title"> <h3 id="header_message">'.urldecode($form['header_message']).'</h3></div> <div id="form_messages" class="submit_message" ></div>';
        ...
        $out.='<button id="sending" class="btn btn-primary btn-lg btn-send2" style="display: none;">'.__('Sending ...','tfuse').'</button>
            <button type="submit" id="send_form" class="btn btn-primary btn-lg btn-submit2" name="submit" >'. urldecode($form['submit_mess']).'</button>
...
function textarea($input,$form_input){
    return "<div class='row' style='".$form_input['floating']."'>
                <label class='label_title' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'><strong>".$form_input['label_text']."</strong></label>
...
function captcha($input,$form_input){
    ...
    $out="<div class='row alignleft field_text' style='width:".($form_input['ewidthpx'])."px;".$form_input['floating']."'>
            <label class='label_title' for='" .TF_THEME_PREFIX.'_'.trim($form_input['shortcode']). "'><strong>".$form_input['label_text']."</strong></label>
...
function select($input,$form_input){
    ...
    $out = "<div class='row field_select alignleft ".$form_input['classes']."' style='".$form_input['floating']."'>
                <label class='label_title' for='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "'><strong>".$form_input['label_text']."</strong></label>
                <select  style='width:".($form_input['ewidthpx']+20)."px;' class='select_styled' name='" .TF_THEME_PREFIX."_".trim($form_input['shortcode']). "'>";
    if(is_array($form_input['options'])){
        foreach ($form_input['options'] as $key => $option) {
