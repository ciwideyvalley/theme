...
function tfuse_popular_posts($atts, $content = null)
{
    ...
    foreach ($latest_posts as $post_val):
        ...
        $return_html .= '<div class="extras">' . $post_val['post_img'] .  strip_tags(tfuse_shorten_string(apply_filters('the_content',$post_val['post_content']),17)).' </div>
            <a href="' . $post_val['post_link'] . '" class="link-arrow">' . __('Read more', 'tfuse') . '<span class="icon-caret-right"></span></a>';
        $return_html .= '</li>';
    endforeach;
    $return_html .='</ul></div> ';
